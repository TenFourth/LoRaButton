...
ターミナルを接続して、LoRa Buttonの電源が入るとプロンプトが表示され、コマンド待ちの状態になります。
...
* プログラムが自動的に実行するようになっている場合は、電源ONまたは再起動してから5秒以内に `Ctrl キー` と `c` を同時に押すとプロンプト `>` の状態になります。バージョン番号の下の行に `.` が増えていく状態で `.` が5つになり `OK` が表示されると、プログラムが実行されてプロンプトは表示されません。
* ターミナルに何も表示されない場合は、モジュールがスリープ状態になっている可能性がありますので、[RESET]ピンと[GND]ピンを繋いで再起動させてみてください

## 環境変数

送信出力やデータレート等の設定はターミナルからコマンドを入力して、環境変数を変更します。製品出荷時はデフォルト値より以下の値を変更しています。

| 変数名と値 | 説明 |
|--|--|
| `CTRL=$00400020` | JOINの状態をRAMに保存 (電源が切れるとJOINの状態が消去)、LoRa送受信でにLEDを光らせない設定にします |
| `AUTO="PLOAD:RUN"` | 電源ON時にプログラムを自動的に実行します |

環境変数の確認は `#?` を入力して [Enter]
保存は `SSAVE` を入力して [Enter] キーを押します

その他の環境変数、ターミナルの使い方やコマンド一覧は、モジュール製造元（[株式会社アイ・ツー](https://i2-ele.co.jp/)）の [ソフトウェアリファレンスマニュアル](https://www.i2-ele.co.jp/LRA1_software.html) や [LRA1コマンド一覧](https://i2-ele.co.jp/LRA1-command.html) をご参照ください。
...
| アンテナ | 内蔵 |
| 送信/受信周波数 | 920.6MHz ～ 928.0MHz |
| 最大送信出力 | 13.8dBm (13dBm 設定 VDD=3.6V) |
| 受信感度 | -136dBm (LoRa BW=125kHz, SF=12) |
| 製造 | 日本 |
